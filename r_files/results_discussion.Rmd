...
For parameter optimization, there was a focus on making sure there was no overfitting going on. This meant iteration depth was altered for the RandomForest and Logistic algorithm for stopping the algorithm early. More parameters could have been optimized but due to the relatively short timespan, it was decided just the iteration depth would be optimized.
...
Flipper length, body mass, and culmen depth due to their correlation will be useful attributes for the model. Body mass in particular as these attributes were shown to be significantly different within species.
