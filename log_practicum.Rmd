...
```{r}
# Create dataframe with the means of the groups
percentage.correct.final <- setNames(aggregate(adjusted.results$Percent_correct, 
                                         list(adjusted.results$Key_Scheme),
                  FUN=mean), c("algorithm", "mean"))

ggplot(percentage.correct.final, aes(algorithm, mean)) + geom_point() + 
  scale_x_discrete(guide = guide_axis(angle = 90)) + coord_cartesian(ylim = c(0, 100)) + 
  ggtitle("Mean precentage correct of algorithms")
```
...
data.comparison <- matrix(c(percentage.correct$mean[percentage.correct$algorithm == "RandomForest"], percentage.correct.final$mean[percentage.correct.final$algorithm == "RandomForest"], percentage.correct$mean[percentage.correct$algorithm == "Logistic"], percentage.correct.final$mean[percentage.correct.final$algorithm == "Logistic"]), ncol=2)
...
#Turn Key_Scheme into factor for easy plotting
final.results$Key_Scheme <- as.factor(adjusted.results$Key_Scheme) 
#Turn Key_Scheme into factor for easy plotting
